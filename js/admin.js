...
close:function(){this.isOpen=false;var el=document.getElementById("cmd-palette");if(el)el.classList.add("hidden");},
  render:function(q){var results=document.getElementById("cmd-palette-results");if(!results)return;var self=this;var filtered=this.commands.filter(function(c){  if(!q)return true;  return(c.label.toLowerCase().indexOf(q.toLowerCase())>-1)||(c.hint&&c.hint.toLowerCase().indexOf(q.toLowerCase())>-1);});if(filtered.length===0){results.innerHTML='<div class="cmd-empty" style="padding:20px;text-align:center;color:var(--admin-text-dim);font-size:0.88rem;">Aucun résultat</div>';return;}this.activeIdx=0;results.innerHTML=filtered.map(function(c,i){  var cls="cmd-result"+(i===0?" active":"");  var h=c.hint?'<div class="cmd-result-hint">'+c.hint+'</div>':'';  return '<div class="'+cls+'" data-idx="'+i+'"><span class="cmd-result-icon">'+c.icon+'</span><div class="cmd-result-text"><div class="cmd-result-label">'+c.label+'</div>'+h+'</div></div>';}).join("");results.querySelectorAll(".cmd-result").forEach(function(el,i){  el.addEventListener("click",function(){filtered[i].action();self.close();});});},
...
// ==========================================================================
function initNewFeatures(){var cmdInput=document.getElementById("cmd-palette-input");if(cmdInput){cmdInput.addEventListener("input",function(){cmdPalette.render(cmdInput.value);});cmdInput.addEventListener("keydown",function(e){if(e.key==="Escape")cmdPalette.close();if(e.key==="ArrowDown"){e.preventDefault();cmdPalette.navigate(1);}if(e.key==="ArrowUp"){e.preventDefault();cmdPalette.navigate(-1);}if(e.key==="Enter"){e.preventDefault();cmdPalette.execute();}});}var cmdClose=document.getElementById("cmd-close-btn");if(cmdClose){cmdClose.addEventListener("click",function(){cmdPalette.close();});}document.addEventListener("keydown",function(e){if((e.metaKey||e.ctrlKey)&&e.key==="k"){e.preventDefault();if(cmdPalette.isOpen)cmdPalette.close();else cmdPalette.open();}});var fab=document.getElementById("fab-quick");if(fab)fab.addEventListener("click",function(){cmdPalette.open();});var bell=document.getElementById("notif-bell");var dd=document.getElementById("notif-dropdown");if(bell&&dd){bell.addEventListener("click",function(e){e.stopPropagation();dd.classList.toggle("hidden");notifCenter.render();});document.addEventListener("click",function(){dd.classList.add("hidden");});dd.addEventListener("click",function(e){e.stopPropagation();});}var notifClose=document.getElementById("notif-close-btn");if(notifClose){notifClose.addEventListener("click",function(){if(dd)dd.classList.add("hidden");});}var cb=document.getElementById("notif-clear");if(cb)cb.addEventListener("click",function(){notifCenter.clear();});var ov=document.getElementById("cmd-palette");if(ov)ov.addEventListener("click",function(e){if(e.target===ov)cmdPalette.close();});(state.activityLog||[]).slice(0,5).forEach(function(a){notifCenter.add(a.detail,"🔔");});
